...
export function SocialLoginButtons({ onGuestMode }: SocialLoginButtonsProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      toast({
        title: "登录失败",
        description: error.message || "Google登录失败，请重试",
        variant: "destructive",
      });
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      toast({
        title: "登录失败",
        description: error.message || "Apple登录失败，请重试",
        variant: "destructive",
      });
      setLoadingProvider(null);
    }
    ...
    <div className="space-y-3">
      <div className="relative">
        ...
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-card px-2 text-muted-foreground">快捷登录</span>
        </div>
      </div>
