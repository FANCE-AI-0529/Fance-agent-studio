...
const faqs = [
  {
    q: "Agent OS 与 ChatGPT、Claude 等通用大模型有什么区别？",
    a: "Agent OS 不是大模型本身，而是构建在大模型之上的企业级基础设施。它提供容器级隔离（NanoClaw）、多级权限治理（MPLP）、可视化工作流编排、持久化记忆系统和 API 发布能力。您可以将 Agent OS 理解为「AI 员工的操作系统」——大模型是引擎，Agent OS 是整辆车。",
  },
  {
    q: "数据安全如何保障？我的业务数据会被用于模型训练吗？",
    a: "绝对不会。Agent OS 采用零数据留存策略：所有用户数据在独立的 NanoClaw 沙箱中处理，会话结束后即销毁临时数据。持久化数据（如知识库、记忆文件）仅存储在您的专属实例中，完全隔离。企业版支持私有化部署，数据不出您的网络边界。",
  },
  {
    q: "MPLP 协议具体是如何工作的？",
  ...
  },
  {
    ...
    a: "Agent OS 内置支持 GPT-5 系列、Gemini 2.5/3 系列等主流模型，开箱即用无需 API Key。同时支持自定义模型接入——通过标准 OpenAI 兼容接口，您可以接入任何自托管或第三方模型，包括私有化部署的开源模型。",
  },
  {
  ...
  {
    q: "如何获取内测邀请码？",
    a: "目前 Agent OS 处于受邀内测阶段。您可以点击页面上的「获取内测邀请码」按钮提交申请，我们的团队会在 1-3 个工作日内审核并发送邀请码到您的邮箱。企业客户可通过「联系销售」通道获得优先审核。",
  },
  {
    q: "免费版和付费版的核心区别是什么？",
  ...
  },
  {
    ...
    a: "Agent OS 提供标准 RESTful API 和 Webhook 事件订阅，可无缝对接现有系统。每个 Agent 均可一键发布为 API 端点，自动生成密钥和调用统计。SDK 支持 Python、TypeScript、Go 等主流语言。迁移通常只需几小时即可完成核心对接。",
  },
];
