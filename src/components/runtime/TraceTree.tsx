...
const eventConfig: Record<TraceEventType, EventConfig> = {
  intent_detected: {
    icon: Brain,
    label: "意图识别",
    ...
    <div className="flex gap-2">
      ...
      <div className="flex-1 pb-2 min-w-0">
        ...
        <div className="mt-0.5 space-y-0.5">
          ...
          {event.data.intent && (
            <p className="text-[10px] text-muted-foreground truncate">"{event.data.intent}"</p>
          )}
          ...
          {event.data.reason && (
            <p className="text-[10px] text-muted-foreground truncate">{event.data.reason}</p>
          )}
          {event.data.message && (
            <p className="text-[10px] text-destructive truncate">{event.data.message}</p>
          )}
          {event.data.tokenCount !== undefined && (
            <span className="text-[9px] text-muted-foreground">Tokens: {event.data.tokenCount}</span>
          )}
          {event.data.duration !== undefined && (
            <span className="text-[9px] text-muted-foreground">耗时: {event.data.duration}ms</span>
          )}
        </div>
      </div>
    </div>
...
function TraceSessionItem({ session, filteredEvents, isActive }: TraceSessionItemProps) {
  const [isExpanded, setIsExpanded] = useState(isActive);

  // Sync expanded state with active session
  useEffect(() => {
  ...
  return (
    <div
      className={cn(
        ...
        isActive ? "border-primary/50 bg-primary/5 shadow-sm" : "border-border bg-card hover:bg-secondary/30"
      )}
    >
      {/* Header */}
      <button className="w-full p-2 flex items-center gap-1.5 text-left" onClick={() => setIsExpanded(!isExpanded)}>
        ...
        <span className="text-xs font-medium flex-1 truncate min-w-0">{session.query || "新会话"}</span>
      ...
      {!isExpanded && session.events.length > 0 && (
        <div className="px-2 pb-1.5 flex items-center gap-2 text-[9px] text-muted-foreground">
          ...
          {eventCounts.success > 0 && <span className="text-status-executing">✓{eventCounts.success}</span>}
          {eventCounts.failed > 0 && <span className="text-destructive">✗{eventCounts.failed}</span>}
        </div>
      )}
          ...
          {filteredEvents.length > 0 ? (
            <div className="pl-1">
              {filteredEvents.map((event, index) => (
                <TraceEventItem key={event.id} event={event} isLast={index === filteredEvents.length - 1} />
              ))}
            </div>
          ) : (
            <p className="text-[10px] text-muted-foreground py-1.5 text-center">
              {session.events.length > 0 ? "已过滤所有事件" : "暂无事件"}
            </p>
          )}
        </div>
      )}
...
export function TraceTree({ sessions, currentSessionId, onClearSessions, onRefresh }: TraceTreeProps) {
  const [activeTab, setActiveTab] = useState<"all" | "running" | "completed" | "failed">("all");
  const [showEventTypes, setShowEventTypes] = useState<Set<TraceEventType>>(
    new Set(Object.keys(eventConfig) as TraceEventType[])
  );

  // Filter sessions by status tab
      ...
      <div className="flex items-center justify-between gap-1 mb-2">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)} className="flex-1">
          <TabsList className="h-7 p-0.5 grid grid-cols-4 gap-0.5">
            <TabsTrigger
              value="all"
              className="text-[10px] h-6 px-1.5 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
            >
              全部
            </TabsTrigger>
            <TabsTrigger
              value="running"
              className="text-[10px] h-6 px-1.5 data-[state=active]:bg-status-planning data-[state=active]:text-primary-foreground"
            >
              运行
            </TabsTrigger>
            <TabsTrigger
              value="completed"
              className="text-[10px] h-6 px-1.5 data-[state=active]:bg-status-executing data-[state=active]:text-primary-foreground"
            >
              完成
            </TabsTrigger>
            <TabsTrigger
              value="failed"
              className="text-[10px] h-6 px-1.5 data-[state=active]:bg-destructive data-[state=active]:text-destructive-foreground"
            >
              失败
            </TabsTrigger>
          </TabsList>
          ...
          {filteredSessions.length > 0 ? (
            filteredSessions.map((session) => (
              <TraceSessionItem
                key={session.id}
                session={session}
                filteredEvents={session.events.filter((e) => showEventTypes.has(e.type))}
            ...
            <div className="text-center py-6 text-muted-foreground">
              ...
              <p className="text-xs">无符合条件的会话</p>
            </div>
          )}
        </div>
      </ScrollArea>
